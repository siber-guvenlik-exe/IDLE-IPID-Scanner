...
from scapy.all import IP, TCP, Ether, sr1, send, sendp
import argparse
...
parser.add_argument('-zp', '--zombieport',required=True, help="Port to use the zombie from")      # option that takes a value
parser.add_argument('-s', '--sourceport',required=False, default=80, help="Source port to send the packets from, its optional but can help to evade firewalls")
...
def send_synack_to_zombie(zombie_ip):
    while True:
        ip = IP(dst=zombie_ip,)
        tcp = TCP(
        ...
        ack=1000            # Acknowledgment number
    )
        payload = "Hello, this is a test."  # Add some data to the TCP payload
...
def send_syn_to_target(target_ip, dport, spoof_ip):
    ip = IP(dst=target_ip, src=spoof_ip)
    tcp = TCP(
...
try:
    ...
    if (args.ports == "ALL"):
        ...
        port = [
    1, 3, 7, 9, 13, 17, 19, 20, 21, 22, 23, 25, 26, 37, 53, 69, 79, 80, 81, 82,
    83, 84, 85, 88, 89, 90, 99, 100, 106, 109, 110, 111, 113, 119, 125, 135,
    139, 143, 144, 146, 161, 163, 179, 199, 211, 212, 222, 254, 255, 259, 264,
    280, 301, 306, 311, 340, 366, 389, 406, 407, 416, 417, 425, 427, 443, 444,
    458, 464, 465, 481, 497, 500, 512, 513, 514, 515, 524, 541, 543, 544, 545,
    548, 554, 555, 563, 587, 593, 616, 617, 625, 631, 636, 646, 648, 666, 667,
    668, 683, 687, 691, 700, 705, 711, 714, 720, 722, 726, 749, 765, 777, 783,
    787, 800, 801, 808, 843, 873, 880, 888, 898, 900, 901, 902, 903, 911, 912,
    981, 987, 990, 992, 993, 995, 999, 1000, 1001, 1002, 1007, 1009, 1010, 1011,
    ...
    1093, 1094, 1095, 1096, 1097, 1098, 1099, 1100, 1102, 1104, 1105, 1106,
    1107, 1108, 1110, 1111, 1112, 1113, 1114, 1117, 1119, 1121, 1122, 1123,
    1124, 1126, 1130, 1131, 1132, 1137, 1138, 1139, 1140, 1141, 1145, 1147,
    ...
    1583, 1594, 1600, 1641, 1658, 1666, 1687, 1688, 1700, 1717, 1718, 1720,
    1721, 1723, 1755, 1761, 1782, 1783, 1801, 1805, 1812, 1839, 1862, 1863,
    ...
    2191, 2196, 2200, 2222, 2251, 2260, 2288, 2301, 2323, 2332, 2333, 2334,
    2351, 2366, 2381, 2382, 2393, 2394, 2399, 2401, 2492, 2500, 2522, 2525,
    2557, 2601, 2602, 2604, 2605, 2607, 2608, 2638, 2701, 2702, 2710, 2717,
    2718, 2725, 2800, 2809, 2811, 2869, 2875, 2909, 2910, 2920, 2967, 2998,
    3000, 3001, 3003, 3005, 3006, 3007, 3011, 3013, 3017, 3030, 3031, 3050,
    3052, 3071, 3077, 3127, 3128, 3168, 3211, 3214, 3233, 3260, 3268, 3283,
    3300, 3306, 3323, 3324, 3325, 3333, 3351, 3367, 3369, 3370, 3371, 3389,
    3390, 3404, 3476, 3493, 3517, 3527, 3546, 3551, 3580, 3659, 3689, 3690,
    3703, 3737, 3766, 3784, 3800, 3801, 3809, 3814, 3826, 3827, 3828, 3851,
    3863, 3871, 3878, 3880, 3889, 3905, 3914, 3918, 3920, 3945, 3971, 3986,
    3995, 3998, 4000, 4001, 4002, 4003, 4004, 4005, 4045, 4111, 4125, 4126,
    4129, 4224, 4242, 4279, 4321, 4343, 4443, 4444, 4445, 4446, 4449, 4550,
    4567, 4662, 4848, 4899, 4900, 4998, 5000, 5001, 5002, 5003, 5004, 5009,
    5030, 5033, 5050, 5051, 5054, 5060, 5061, 5080, 5087, 5100, 5101, 5120,
    5190, 5200, 5214, 5221, 5222, 5225, 5226, 5269, 5280, 5298, 5357, 5405,
    5414, 5431, 5432, 5440, 5500, 5510, 5544, 5550, 5555, 5556, 5557, 5560,
    5566, 5631, 5633, 5666, 5678, 5679, 5718, 5730, 5800, 5801, 5802, 5810,
    5811, 5820, 5862, 5900, 5901, 5902, 5903, 5904, 5906, 5907, 5910, 5911,
    5915, 5922, 5925, 5950, 5952, 5959, 5960, 5961, 5962, 5987, 5988, 5989,
    ...
    6692, 6699, 6779, 6788, 6792, 6839, 6881, 6901, 6969, 7000, 7001, 7002,
    7004, 7007, 7019, 7025, 7070, 7100, 7103, 7106, 7200, 7402, 7435, 7443,
    ...
    8088, 8089, 8090, 8093, 8097, 8100, 8180, 8181, 8192, 8193, 8194, 8200,
    8222, 8254, 8290, 8291, 8292, 8300, 8333, 8383, 8400, 8402, 8443, 8500,
    8600, 8649, 8651, 8652, 8654, 8701, 8800, 8873, 8880, 8883, 8888, 8910,
    8989, 8994, 9000, 9001, 9002, 9003, 9009, 9010, 9040, 9050, 9071, 9080,
    9081, 9090, 9091, 9099, 9100, 9101, 9102, 9103, 9110, 9111, 9119, 9125,
    9160, 9191, 9200, 9207, 9220, 9290, 9415, 9418, 9443, 9535, 9575, 9593,
    9594, 9618, 9666, 9876, 9877, 9878, 9898, 9899, 9988, 9999, 10000, 10001,
    ...
    13783, 14000, 14238, 14441, 14442, 15000, 15002, 15003, 15660, 15742,
    16000, 16001, 16012, 16016, 16018, 16080, 16113, 16992, 16993, 17877,
    17988, 18040, 18101, 18988, 19101, 19801, 19842, 20000, 20005, 20031,
    ...
    33354, 33899, 34571, 34572, 34573, 35500, 38292, 40193, 48080, 49152,
    49153, 49154, 49155, 49156, 49157, 49158, 49159, 49160, 49161, 49163,
    49165, 49167, 49175, 49176, 49400, 49999, 50000, 50001, 50002, 50003,
    50006, 50300, 50389, 50500, 50636, 50800, 51103, 51493, 52869, 54045,
    54328, 55055, 55555, 55600, 56737, 56738, 57294, 57797, 58080, 60020,
...
for i in port:
    ...
    send_syn_to_target(args.target, i, zombie)
    second_ipid = send_synack_to_zombie(zombie)

    #print(f"First IP ID: {first_ipid}")
